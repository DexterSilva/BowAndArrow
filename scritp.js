...
const randomX = Math.floor(Math.random() * maxX); // Atualiza a posição horizontal (esquerda) do alvo em pixels
...
function countdown() { //Declara a função que diminui o tempo e verifica o fim do jogo
    ...
    if (time <-0) { //Verifica se o tempo acabou(chegou á zero ou menor)
clearInterval(gameinterval); // Para o intervalo que contrala a contagem do tempo
...
alert("Fim de jogo! Sua pontuação foi: " + score); //Mostra uma janela com a pontuação atual
    }
